...
namespace OpenIDE.Library.Console
{
    ...
    public class FirebugConsole
    {
        ...
        {
            Output = new StandardConsoleOutput();
        }
        ...
        public void Assert(bool expression, params object[] items)
        {
            if (expression == false)
            {
                if (items.Length > 0)
                {
                    ...
                    var formattedItems2 = new object[formattedItems.Length + 1];
        ...
        {
            output.StartGroup(Format(items), false);
        }
        ...
        {
            output.StartGroup(Format(items), true);
        }
        ...
        public void Time([Optional] string name)
        {
            if (name == null)
                name = string.Empty;
            if (timers == null)
        ...
        public void TimeEnd([Optional] string name)
        {
            if (name == null)
                name = string.Empty;
            if (timers == null || timers.ContainsKey(name) == false)
                return;
        ...
        {
            output.Log(style, FormatObjects(items));
        }
        ...
        private static object[] FormatObjects(object[] items)
        {
            ...
            {
                var formatString = (string) items[0];
                ...
                while (items.Length > itemsConsumed)
                {
                    ...
                    switch (patternType)
                    {
                        ...
                        case 'd':
                        case 'i':
                            double number = Convert.ToDouble(items[itemsConsumed++]);
                            replacement =
                                (number >= 0 ? System.Math.Floor(number) : System.Math.Ceiling(number)).ToString();
                            break;
                        case 'f':
                            replacement = Convert.ToDouble(items[itemsConsumed++]).ToString();
                            break;
                        case '%':
                            replacement = "%";
                            break;
